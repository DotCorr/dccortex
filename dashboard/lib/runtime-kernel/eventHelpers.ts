...
export function parseEventSteps(raw: unknown): EventActionConfig[] {
  ...
  if (typeof raw === 'object') {
    ...
    if (Array.isArray(obj.steps)) {
      return obj.steps.map((s) => parseEventConfig(s)).filter(Boolean) as EventActionConfig[]
    }
    const one = parseEventConfig(obj)
    return one ? [one] : []
  }
  ...
  if (typeof raw === 'string') {
    ...
    if (s.startsWith('{')) {
      try {
        ...
        if (Array.isArray(obj.steps)) {
          return obj.steps.map((st) => parseEventConfig(st)).filter(Boolean) as EventActionConfig[]
        }
        const one = parseEventConfig(obj)
        return one ? [one] : []
      } catch {
 ...
 * Single-step: uses old single-config format for backward compatibility.
...
export function eventHelpersSummary(): string {
  const lines: string[] = []
  for (const [group, items] of Object.entries(EVENT_HELPERS)) {
    lines.push(`${group}:`)
    for (const i of items) lines.push(`- ${i.name}: ${i.desc}`)
  }
  lines.push('Condition ops: ' + CONDITION_OPS.map((o) => o.value).join(', '))
  return lines.join('\n')
}
