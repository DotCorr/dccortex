...
import type { AnimationSequenceConfig, AnimationStep } from './animationTypes'
...
export async function executeAnimationSequence(
  ...
  const playParallelOnce = async (): Promise<void> => {
    const animations = sequence.steps.map((step: AnimationStep) => buildAnimationString(step)).join(', ')
